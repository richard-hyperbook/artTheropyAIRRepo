...
/*
  ...
  backupHyperbook() async {
    models.FileList? backupFileList =
        await listStorageFiles(bucketId: backupStorageRef.path);
    int maxVersion = 0;
          ...
          body: SafeArea(
            child: SingleChildScrollView(
              child: Column(children: <Widget>[
                ...
                if (localDB.getWorkingHyperbook().moderator!.path ==
                            ...
                            Expanded(
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder:
                                  ...
                                  final ConnectedUsersRecord
                                      listViewConnectedUsersRecord = localDB
                                          .getWorkingHyperbookLocalDB() /*#cachedHyperbookList[currentCachedHyperbookIndex!]*/
                                          .connectedUserList[listViewIndex];

                                  String requesting =
                                      listViewConnectedUsersRecord.requesting!;
                                  //chosenRole = listViewConnectedUsersRecord.status!;
                                  bool isRequesting = (requesting != '');
                                  print(
                                      '(HE2)${requesting}````${listViewConnectedUsersRecord.reference!.path}++++${listViewConnectedUsersRecord.displayName}>>>>${listViewConnectedUsersRecord.status}');
                                  return Material(
                                      color: Colors.transparent,
                                      elevation: 5.0,
                                      child: Container(
                                        ...
                                        child: isRequesting
                                            ? InkWell(
                                                                      ...
                                                                      onPressed:
                                                                        ...
                                                                        await localDB.updateConnectedUser(
                                                                            hyperbookIndex: localDB
                                                                                .workingHyperbookIndex,
                                                                            connectedUserIndex:
                                                                                listViewIndex,
                                                                            cp:
                                                                                kAttConnectedUserStatus,
                                                                            value:
                                                                                requesting);
                                                                        await localDB.updateConnectedUser(
                                                                            hyperbookIndex: localDB
                                                                        ...
                                                                        /*#await updateDocument(
                                                                            collection:
                                                                                connectedUsersRef,
                                                                            document:
                                                                                listViewConnectedUsersRecord.reference,
                                                                            data: {
                                                                              'requesting': '',
                                                                              'status': requesting,
                                                                            });*/
                                                                        UsersRecord
                                                                            user =
                                                                            await getUser(document: listViewConnectedUsersRecord.user!);
                                                                        /*# await setCachedHyperbookAndChapterList(
                                                                            widget.hyperbook!);*/
                                                                        sendEmail(
                                                                            emailType:
                                                                                EmailType.roleGrant,
                                                                            senderDisplayName: currentUser!.displayName!,
                                                                            senderEmail: currentUser!.email!,
                                                                            hyperbookName: localDB.getWorkingHyperbook().title!, //#widget.hyperbookTitle!,
                                                                            receiverEmail: user.email!,
                                                                            receiverDisplayName: user.displayName!,
                                                                            newRole: requesting);
                                                                        toast(
                                                                            context,
                                                                            'Role of ${listViewConnectedUsersRecord.displayName} updated to ${requesting} for ${_model.titleController.text}',
                                                                            ToastKind.success);

                                                                        //%print(
                                                                        //     '(N3600)${requesting}####${getCurrentHyperbook().reference}');
                                                                        setState(
                                                                            () {
                                                                          // localDB
                                                                          //     .getWorkingHyperbookLocalDB()
                                                                          //     .connectedUserList[listViewIndex]
                                                                          //     .status = requesting;
                                                                          // localDB
                                                                          //     .getWorkingHyperbookLocalDB() /*#cachedHyperbookList[currentCachedHyperbookIndex!]*/
                                                ...
                                                child: Text(
                                                  '${listViewConnectedUsersRecord.displayName} : ${listViewConnectedUsersRecord.status}, requesting; ${requesting}',
                                                                      ...
                                                                      return AlertDialog(
                                                                        ...
                                                                        content:
                                                                          ...
                                                                          value:
                                                                              chosenRole,
                                                                          hint:
                                                                              const Text('Please select hyperbook role'),
                                                                          items:
                                                                              kRoleListWithoutModerator.map<DropdownMenuItem<String>>((String item) {
                                                                            return DropdownMenuItem<String>(
                                                                              enabled: (_checkRoleLegal(item: item, currentRole: listViewConnectedUsersRecord.status!)),
                                                                              value: item,
                                                                              child: Text(item, style: TextStyle(color: (_checkRoleLegal(item: item, currentRole: listViewConnectedUsersRecord.status!) ? Colors.black : Colors.grey))),
                                                                            );
                                                                          }).toList(),
                                                                          elevation:
                                                                              2,
                                                                        ...
                                                                        actions: [
                                                                          ...
                                                                          TextButton(
                                                                            onPressed:
                                                                                () async {
                                                                              /*#await updateDocument(collection: connectedUsersRef, document: listViewConnectedUsersRecord.reference, data: {
                                                                                'requesting': '',
                                                                                'status': chosenRole,
                                                                              });*/
                                                                              /*#await setCachedHyperbookAndChapterList(widget.hyperbook!);*/
                                                                              await localDB.updateConnectedUser(hyperbookIndex: localDB.workingHyperbookIndex, connectedUserIndex: localDB.getConnectedUserIndex(listViewConnectedUsersRecord.reference!), cp: kAttConnectedUserRequesting, value: '');
                                                                              await localDB.updateConnectedUser(
                                                                                hyperbookIndex: localDB.workingHyperbookIndex,
                                                                                connectedUserIndex: localDB.getConnectedUserIndex(listViewConnectedUsersRecord.reference!),
                                                                                cp: kAttConnectedUserStatus,
                                                                                value: chosenRole,
                                                                              );
                                                                              setState(() {});
                                                                              toast(context, 'Role of ${listViewConnectedUsersRecord.displayName} updated to ${chosenRole} for ${_model.titleController.text}', ToastKind.success);
                                                                              Navigator.pop(context, false);
                                                                              //%print(
                                                                              //     '(N3600A)${requesting}####${getCurrentHyperbook().reference}');
                                                                              // localSetState(() {
                                                                              //#cachedHyperbookList[currentCachedHyperbookIndex!].connectedUserList[listViewIndex].status = chosenRole;
                                                                              // localDB.hyperbooklocalDBList[localDB.workingHyperbookIndex!].connectedUserList[listViewIndex].status = chosenRole;
                                                                              //# cachedHyperbookList[currentCachedHyperbookIndex!].connectedUserList[listViewIndex].requesting = '';
                                                                              // localDB.hyperbooklocalDBList[localDB.workingHyperbookIndex!].connectedUserList[listViewIndex].requesting = '';
                                                                              // });
                                                                              // context.pop();
                                                                            },
                                                                            child:
...
bool _checkRoleLegal({required String item, required String currentRole}) {
  //>print('<LD71>${item}');
  if ((currentRole == kRoleModerator) && (item == kRoleModerator)) {
    return true;
  }
  if (currentRole != kRoleModerator) {
    return true;
  }
  return false;
}
